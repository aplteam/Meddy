...
⍝ Kai Jaeger ⋄ APL Team Ltd
    ...
    ∇ History
      ...
      ⍝ * Bug fixes:
      ⍝ ##### 3.4.0
      ⍝ * `markdownStrinct` has a changed meaning: when this is 1 (default is 0) then:
      ⍝   * A list can only start after a blank line.
      ⍝   * Footers in tables are not recognized any more.
      ⍝ * What was previously `markdownStrinct` is now `syntaxSugar` with negated default.
      ⍝ * Documentation polished.
      ⍝ * Problem with in-line markup within blockquotes fixed.
      ⍝ * Bug fixes:
      ⍝   * In case `cssUrl` does not carry a trailing `\` (local URL on Windows) or `/` (everything else)
      ⍝     the appropriate one is added.
      ⍝   * Invalid parameter definitions like `[parm]foo=` made MarkAPL crash.
      ⍝   * Markdown in-line mark-up within link text was not converted.
      ⍝ ##### 3.3.0
      ⍝ * Table now can have footers
      ⍝ * Bug fixes
      ⍝   * ATX headers should have allowed 0 to 3 blanks at the start of a line but did not.
      ⍝ ##### 3.2.0
...
⍝                                  (¯1↑html)←⊂buff,'</li>'
    ...
    ∇ r←ns ProcessInlineMarkUp tx
      ...
      r←ns SmartStuff⍣(⊃ns.parms.syntaxSugar)⊣r
      r←(,¨'<>')⎕R'\&lt;' '\&gt;'⊣r                 ⍝ Needed!
      r←ProcessImages r
    ...
    ∇ txt←{ignoreURLs}ProcessUnderscores txt_;noOf;bool;ind;start;end;txt2;b
    ...
    ⍝ * within the URL
      ...
      :AndIf 0<+/bool←bool\'\'≠txt[¯1+Where bool]
          ...
          :If ignoreURLs
⍝              bool∧←(⎕UCS 9)≠'<a .*>' '</a>' '_'⎕R(,¨'&&',(⎕UCS 9))⍠('Greedy' 0)⊣txt  ⍝ Leave `_` within URLs alone   ⍝TODO⍝
              bool∧←bool{0∊⍴⍵:⍺ ⋄ r←(⍴⍺)⍴0 ⋄ r[⊃,/{⍵[1]+⍳⍵[2]}¨⍵]←0 ⋄ r}'<a .*>*.</a>'⎕S 0 1⍠('Greedy' 0)⊣(~bool){⍺\⍺/⍵}txt
          :EndIf
    ...
    ∇
      ...
      ProcessLinks←{
          ...
          sa←GetSpecialAttributes⌽{'{'∊⍵:{⍵↑⍨⍵⍳'{'}{⍵↓⍨¯1+⍵⍳'}'}⍵ ⋄ ''}⌽mask/linkDef   ⍝ Because special attributes for a link follow traight after the link
          txt←off↓txt
          mask←⌽~GetMaskForCode linkDef
      ...
      ProcessInlineMarkupInLinkText←{
          ...
          r←ProcessDoubleTildes r
          ns SmartStuff⍣(⊃ns.parms.syntaxSugar)⊣r
⍝          '`.*`' '\_' '\*' '\~\~'⎕R'&' '\\_' '\\*' '\\~\\~'⍠('Greedy' 0)⊣⍵  ⍝TODO⍝  OBSOLETE?!
      }
    ...
    ∇ ns←ScanMarkdown ns;∆LastLineWasEmpty
      ∆LastLineWasEmpty←0
      :Repeat
